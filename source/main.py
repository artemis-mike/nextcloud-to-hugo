...
def sanitize_branch_name(name):
    # Branch name should be a safe string
    target = re.sub(r'[^a-zA-Z0-9_\-]', '-', name)
    return 'add-post-' + target[:40].strip('-')

def sanitize_dir_name(date, name):
    name_clean = re.sub(r'[^a-zA-Z0-9_\-]', '_', name)
    return f"{date}_{name_clean}"
...
def main():
    ...
    for year, articles in articles_by_year.items():
        for article in articles:
            ...
            branch_name = sanitize_branch_name(article['name'])
            ...
            if doc_file:
                ...
                logging.warning(f"No supported document found for {article['name']}, will create empty body.")
            ...
            # Commit and push
            if hugo_gen.commit_and_push(f"Add post {title}", branch_name):
                # Create PR
